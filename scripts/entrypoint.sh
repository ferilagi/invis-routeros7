...
# Copy source if file not present
if [[ ! -e "/routeros/generate-dhcpd-conf.py" ]]; then
   cp -r /routeros_source/. /routeros
fi

# Cek ulang apakah file sudah ada
if [[ ! -f "/routeros/generate-dhcpd-conf.py" ]]; then
   echo "generate-dhcpd-conf.py not found! Exiting."
   ls -la /routeros
   exit 1
fi
...
# Deteksi KVM hanya jika arsitektur x86_64
if [[ "$ARCH" == "x86_64" ]]; then
   if [ -e /dev/kvm ]; then
   ...
   echo "Non-x86 architecture detected ($ARCH), skipping KVM and CPU host features."
fi

if [ "$CPU_FEATURES" = "" ]; then
   echo "KVM not available or disabled, running in emulation mode. This will be slow."
fi
...
exec qemu-system-x86_64 \
   ...
   $( [[ "$ARCH" == "x86_64" ]] && echo "-cpu host$CPU_FEATURES $KVM_OPTS" ) \
